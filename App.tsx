...
const App: React.FC = () => {
  ...
  const renderPage = () => {
    ...
    switch (currentPage) {
      ...
      case 'daily-route': return <DriverDailyRoute session={session!} user={currentUser} customers={customers} onBack={() => navigate('operation')} onSubmit={(dr) => { navigate('operation'); setTimeout(() => { try { saveRecord(setDailyRoutes, dr); } catch (_) { /* falha silenciosa; rota pode ser lançada de novo */ } }, 0); }} />;
