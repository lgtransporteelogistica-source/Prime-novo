...
export const AdminDriverLive: React.FC<AdminDriverLiveProps> = ({ users, onBack }) => {
  ...
  const motoristasMap = new Map(users.filter(u => u.perfil === UserRole.MOTORISTA).map(u => [u.id, u]));
    ...
    <div className="space-y-6 animate-fadeIn max-w-4xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Tempo real via GPS do celular. Atualiza automaticamente.</p>
        </div>
      ...
      {!supabase && (
        <Card className="border-slate-800">
          <p className="text-slate-400 text-sm">Defina VITE_SUPABASE_URL e VITE_SUPABASE_ANON_KEY no .env para ativar o monitoramento.</p>
        </Card>
      )}
      ...
      {supabase && (
        <Card className="border-slate-800">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-black uppercase tracking-widest text-white">Motoristas com localização ativa</h3>
          </div>
          {loading ? (
            <p className="text-slate-500 text-sm">Carregando...</p>
          ) : list.length === 0 ? (
            <p className="text-slate-500 text-sm">Nenhum motorista enviou localização ainda. Peça para o motorista abrir o app no celular e permitir acesso à localização.</p>
          ) : (
            <ul className="space-y-3">
              {list.map((loc) => (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => openInMaps(loc.lat, loc.lng, loc.userName)}
